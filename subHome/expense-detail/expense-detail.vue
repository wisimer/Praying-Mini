<template>
	<view>
		...
		<view class="tabs-box" v-if="showTab">
			<view class="tabs">
				...
				<view class="flex-1">
					...
					<view class="record-item margin-b20">
						...
						<view class="flex flex-wrap">
							<view class="font-size28" :class="item.remark ? 'margin-b10' : ''" style="width: 50%;">
								<text class="color-8C8888">记录类型：</text>
								<text>{{item.typeInfo[0].name}}</text>
							</view>
							<view class="font-size28" :class="item.remark ? 'margin-b10' : ''" style="width: 50%;">
								<text class="color-8C8888">金额：</text>
...
<script setup>
	...
	// getMonthBoundaries
	onLoad(({ type_id, month, monthtype }) => {
		if (type_id) {
	...
	const updateDel = (e) => {
		...
		let url =
			`/subHome/create-record/create-record?type=1`
